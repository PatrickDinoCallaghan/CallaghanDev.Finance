...
using static CallaghanDev.Finance.BlackScholes;
namespace CallaghanDev.Finance
{
    ...
    public class BlackScholes
    {
        ...
        {
            _volatility = new ThreadSafeDecimal();
            _maturity = maturity;
        ...
        {
            double numerator = System.Math.Log((double)(S / K)) + (double)((_riskFreeRate + (sigma * sigma) / 2m) * t);
            double denominator = (double)sigma * System.Math.Sqrt((double)t);
        ...
        {
            double nd1Prime = (1.0 / System.Math.Sqrt(2 * System.Math.PI)) * System.Math.Exp(-0.5 * d1 * d1);
            return S * (decimal)nd1Prime * (decimal)System.Math.Sqrt((double)t);
        }
            ...
            for (int i = 0; i < maxIterations; i++)
            {
                ...
                if (option.Type == OptionType.Call)
                {
                    calculatedPrice = option.AssetPrice * (decimal)Function.NormalCdf(d1)
                                      - option.StrikePrice * (decimal)System.Math.Exp((double)(-_riskFreeRate * t)) * (decimal)Function.NormalCdf(d2);
                }
                else
                {
                    calculatedPrice = option.StrikePrice * (decimal)System.Math.Exp((double)(-_riskFreeRate * t)) * (decimal)Function.NormalCdf(-d2)
                ...
                if (System.Math.Abs(priceDifference) < epsilon)
                {
                    return sigma;
                }
        ...
        public decimal CalculateCallPrice(decimal assetPrice, decimal strikePrice)
        {
            ...
            return assetPrice * (decimal)Function.NormalCdf(d1)
                   - strikePrice * (decimal)System.Math.Exp((double)(-_riskFreeRate * t)) * (decimal)Function.NormalCdf(d2);
        }
        ...
        public decimal CalculatePutPrice(decimal assetPrice, decimal strikePrice)
        {
            ...
            return strikePrice * (decimal)System.Math.Exp((double)(-_riskFreeRate * t)) * (decimal)Function.NormalCdf(-d2)
